...
def generate_hardware_config(config, steppers, mcu_name):
  ...
  if temp_parser.has_section('cfggen_source'):
    if temp_parser.get('cfggen_source', 'source', None) == 'url':
      source_url = temp_praser.get('cfggen_source', 'url')
...
'''
  ...
  s.seek(0)
  full_config = s.read()
  full_config = f"""# This configuration is auto-generated by the 3MS Configuration Generator
